...
class ChatViewModel extends ChangeNotifier {
  ...
  Future<void> initializeChatService() async {
    ...
    _stompClient = StompClient(
      config: StompConfig(
        url: 'ws://${Constants.chat_server}/chat-service/wsstomp',
        onConnect: (frame) {
          completer.complete();
        },
    ...
    final roomListResponse = await http.get(
      Uri.parse("http://${Constants.chat_server}/chat-service/findrooms"),
      headers: {
    ...
    if (roomListResponse.bodyBytes.isNotEmpty) {
      ...
      for (ChatRoom chatRoom in _chatRoomList) {
        var chatMessageListData = await http.get(
          Uri.parse(
              "http://${Constants.chat_server}/chat-service/messagelist/${chatRoom.title}"),
          headers: {
            'Content-Type': 'application/json',
          },
        );
    ...
    _stompClient.subscribe(
        destination: '/sub/chatroomnotify/$email',
        callback: (frame) async {
          ...
          var chatMessageListData = await http.get(
            Uri.parse(
                "http://${Constants.chat_server}/chat-service/messagelist/${newTitle}"),
            headers: {
              'Content-Type': 'application/json',
            },
          );
  ...
  void _updateUnreadCount(String roomTitle) {
    int unreadCount = _chatMessageListForRoom[roomTitle]!.where((chatMessage) => chatMessage.receiver == email && chatMessage.read == false).length;
